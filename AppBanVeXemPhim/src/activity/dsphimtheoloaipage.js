...
export function DSPhimTheoLoaipage({ navigation, route }) {
    const { otherParam } = route.params;
    const [dsphim, setDsphim] = useState([]);
    const [tenloaiphim, setTenloaiphim] = useState('');
    const [link, setLink] = useState('https://rapphimmeme.000webhostapp.com/home/dsphimtheotheloai/' + otherParam);
                    ...
                    if (!ignore) {
                        if (responseJson.dsphim.data == []) {
                            ...
                            if (refreshing) {
                                ...
                                setTenloaiphim(responseJson.lphim.tenLPhim)
                            }
                        }
                    }
                })
        ...
        <View style={{ flex: 1, backgroundColor: '#2c2c2c' }}>
            <View style={{ top: scale(5), flexDirection: 'row', width: '100%' }}>
                ...
                <View style={{ backgroundColor: 'red', padding: scale(5), width: scale(120), height: scale(38), borderRadius: 10, marginBottom: scale(8) }}>
                    <Text style={{ fontSize: scale(18), color: '#2c2c2c', alignSelf: 'center', }}>{tenloaiphim}</Text>
                </View>
            </View>
            <FlatList
